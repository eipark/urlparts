...
class App extends Component {
  ...
  renderJsLocationParts(analyzedUrl) {
    const urlObj = url.parse(analyzedUrl);
    return _.map(JS_LOCATION_PARTS, part => {
      return (
        <li key={part}>
          window.location.{part} = {urlObj[part]}
        </li>
      );
    });
  }
